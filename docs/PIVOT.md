...
The pivot was executed in the sibling repo:
`~/projects/active/hyperion/raincg/`. Two corrections to this
document's framing, both evidence-forced:

1. **The moat test failed for the VSA substrate as such.** The
   authoritative verdict is `hyperion/raincg/DECISION.md` (2026-05-31):
...
2. **The thesis that survives (and is now fresh-measured):** *learned
   ...
   scoreboard with evidence tiers: `hyperion/raincg/RESULTS.md`
...
(publish) remains gated on the owner: the PAPER.md hold's patent
rationale is stale (no RAIN patent claims pending) — publication is now
purely a strategic call. The open research frontier is COGS's 12 hard
cross-construction categories (templating ceiling = 0%); design at
`hyperion/raincg/COGS-ATTACK-DESIGN.md`.
