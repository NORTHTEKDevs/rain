...
- **Success:** a defensible research contribution: exact compositional
  generalization, measured honestly against strong baselines, cheap to
  reproduce, auditable end to end.
...
## Status update — 2026-07-11 (where the pivot landed)

The pivot was executed under `raincg/` in this repo. Two corrections to
this document's framing, both evidence-forced:

1. **The moat test failed for the VSA substrate as such.** The
   authoritative verdict is `raincg/DECISION.md` (2026-05-31):
   the one clean cross-construction COGS win was produced by
   construction-keyed role binding in pure Python — VSA contributed
   nothing to the generalization. VSA is an exact, auditable, lossless
   transducer *given* structure, not the source of it.
2. **The thesis that survives (and is now fresh-measured):** *learned
   selection/perception + exact algebraic, content-independent
   composition* (the hybrid) achieves 100% on real SCAN compositional
   splits where a matched from-scratch transformer and local LLMs
   score ~0%, at orders of magnitude less compute. Head-to-head
   scoreboard with evidence tiers: `raincg/RESULTS.md`
   (generated by `raincg/bench/results_table.py`; raw JSONs in
   `raincg/results/`).

Phase 0/1 of this doc are DONE in corrected form (harness, checkpointed
transformer baseline, LLM baselines, honest-labels table). Phase 2
(publish) remains gated on the owner as a strategic call. The open
research frontier is COGS's 12 hard cross-construction categories
(templating ceiling = 0%); design at `raincg/COGS-ATTACK-DESIGN.md`.
