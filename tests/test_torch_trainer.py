...
def test_context_len_changes_training_dynamics():
    ...
    r_a = train_torch(
        m_a, cb_a, corpus, n_steps=100, batch_size=8, context_len=0, lr=5e-3, device=CPU, seed=0
    )
    r_b = train_torch(
        m_b, cb_b, corpus, n_steps=100, batch_size=8, context_len=4, lr=5e-3, device=CPU, seed=0
    )
...
def test_checkpoint_roundtrip(tmp_path):
    ...
    corpus = "the quick brown fox " * 80
    r = train_torch(
        model, cb, corpus, n_steps=50, batch_size=8, context_len=0, lr=5e-3, device=CPU, seed=0
    )
