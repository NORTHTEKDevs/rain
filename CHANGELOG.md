...
### Added — internal business materials (since removed from the public tree)
...
### Added
- **Resonator + resonant explaining-away readout** at `rain/core/resonator.py` (+ `tests/core/test_resonator.py`, 3 tests). Ported from Hyperion's verified resonance findings, adapted to RAIN's MAP-VSA convention. Adds two capabilities the bind/unbind substrate lacked: (1) `resonator_decode` -- factorize a bound product of UNKNOWN factors (the binding-problem solver; F=3/M=10/D=2048 ~98%), with random-restart + best-reconstruction selection that escapes limit cycles and ~doubles success at the F=4 capacity edge (28% -> 60%); (2) `resonant_extract` -- joint interference-cancelling slot readout. `CompositionalReasoner` gains `compose_sum` + `extract_all`: jointly decoding all slots recovers ~3-4x more slots than per-slot `extract()` under cross-talk. Wired into the N3 chat demo (`examples/01_chat_e2e.py`) as the primary readout (20 slots @ D=256: greedy 16/20, resonant 20/20). No regression on existing compose/SCAN tests.
- **N3-capacity eval** at `evals/tier1_novelty/scan_capacity.py` (+ `tests/test_eval_n3_scan_capacity.py`). Probes the slot-capacity regime the existing N3 (3-5 slots) doesn't reach: at D=256, signed-bundle greedy readout collapses to 0% by 16 slots while resonant `extract_all` holds 100% (and ~85% at 24). Makes the resonant capability measurable in RAIN's own eval suite.
...
- L-NEW-1: Header-audit CI step now exits 1 on missing required file header (was warning-only, exits 0).
