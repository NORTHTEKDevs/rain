...
### Added
- **Resonator + resonant explaining-away readout** at `rain/core/resonator.py` (+ `tests/core/test_resonator.py`, 3 tests). Ported from Hyperion's verified resonance findings, adapted to RAIN's MAP-VSA convention. Adds two capabilities the bind/unbind substrate lacked: (1) `resonator_decode` -- factorize a bound product of UNKNOWN factors (the binding-problem solver; F=3/M=10/D=2048 ~98%); (2) `resonant_extract` -- joint interference-cancelling slot readout. `CompositionalReasoner` gains `compose_sum` + `extract_all`: jointly decoding all slots recovers ~3-4x more slots than per-slot `extract()` under cross-talk. No regression on existing compose/SCAN tests.
