...
class _HymnSampler:
    ...
    def __init__(self, checkpoint_path: str, corpus_path: str) -> None:
        ...
        if saved is not None:
            ...
            self.codebook_matrix = np.stack([cb.vector(c).astype(np.float32) for c in self.vocab])
    ...
    def sample(
        ...
        for c in prompt:
            if c not in self.char_to_idx:
                c = self.vocab[0]
            state = _hymn_forward(state, self._cb.vector(c).astype(np.float32), self.W1, self.W2)
        out_chars: list[str] = []
        recent_ids: list[int] = []
    ...
    while True:
        ...
        if line.startswith("/judge "):
            ...
            if verdict.confidence >= args.min_confidence:
                ...
                print(
                    f"        -> feedback fired, calibration[{relation}] = "
        ...
        if line.startswith("/tally "):
            ...
            print(f"[tally] calibration[{rel}] = " f"{agent.calibration.calibration(rel):.4f}")
            continue
...
def main() -> int:
    ...
    p.add_argument(
        "--kb", default="data/kb_seed/llama3b_expanded.jsonl", help="path to KB seed JSONL"
    )
    p.add_argument(
        "--checkpoint",
    ...
    p.add_argument(
        "--judge-model", default="", help="if set, enables /judge command; e.g. llama3.2:3b"
    )
    p.add_argument(
        "--min-confidence",
        type=float,
    ...
    p.add_argument(
        "--noninteractive", default="", help="for smoke testing: run a single command and exit"
    )
    args = p.parse_args()
