...
def main() -> None:
    ...
    p.add_argument(
        "--warm-start-minilm",
        action="store_true",
        help="Seed the codebook with sentence-transformers MiniLM "
        "embeddings (384-d, ~80 MB). Stronger prior than the "
        "feature variant; mutually exclusive with --warm-start-chars.",
    )
    ...
    if args.warm_start_chars and args.warm_start_minilm:
        raise SystemExit("--warm-start-chars and --warm-start-minilm are mutually exclusive")
    if args.warm_start_chars:
        ws_stats = warm_start_chars(cb, corpus, seed=args.seed)
        print(f"warm-start[features]: {ws_stats}")
    elif args.warm_start_minilm:
        ws_stats = warm_start_chars_minilm(cb, corpus)
        print(f"warm-start[minilm]: {ws_stats}")
    model = HymnTorch(args.in_dim, args.hidden_dim, args.out_dim, seed=args.seed, device=dev)
