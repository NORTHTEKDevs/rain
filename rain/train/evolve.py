...
def mutate(
    genome: np.ndarray, rng: np.random.Generator, sigma: float = 0.1, rate: float = 0.05
) -> np.ndarray:
...
class ChampionState:
    ...
    def credible_lower_bound(
        self, prior_a: float = 1.0, prior_b: float = 1.0, z: float = 1.0
    ) -> float:
...
def should_promote(
    state: ChampionState, threshold: float = 0.6, z: float = 1.0, min_trials: int = 5
) -> bool:
